...
AxesPage::AxesPage(wxBookCtrlBase *parent, LinnStrument * pLinnStrument, const LSSplitType split)
	...
	chkSEND_Y->SetValue(pMyLinnStrument->GetSEND_Y(split));
	...
	chkRELATIVE_Y->SetValue(pMyLinnStrument->GetRELATIVE_Y(split));
	hBoxY->Add(chkRELATIVE_Y, 0, wxEXPAND);
		...
		chkSEND_Z->SetValue(pMyLinnStrument->GetSEND_Z(split));
